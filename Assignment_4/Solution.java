import java.util.Scanner;

class Solution {
    private static final String alphabets = "abcdefghijklmnopqrstuvwxyz";
    private static int N = 0;
    private static int M = 0;
    private static int count = 0;
    private static int minimum_changes = Integer.MAX_VALUE;

    public static void main(String[] args) {
        int n, m;
        Scanner in = new Scanner(System.in);
        n = in.nextInt();
        m = in.nextInt();
        in.nextLine();
        String[] crops = new String[n];
        for (int i = 0; i < n; i++) {
            crops[i] = in.nextLine().trim();
        }
        N = n;
        M = m;
        System.out.println(replant(crops));
        System.out.println(count);
    }

    private static boolean isInValid(int x, int y) {
        return (x < 0 || x >= N || y < 0 || y >= M);
    }

    private static void helper(String[] crops, int x, int y, char c, int changes) {
        count++;
        if (crops[x].charAt(y) != c) {
            changes++;
        }
        if (changes >= minimum_changes) {
            return;
        }
        String new_string = crops[x].substring(0, y) + c + crops[x].substring(y + 1);
        String old_string = crops[x];
//        System.out.println(new_string + " / " + old_string);
        crops[x] = new_string;
        int current_row = x;
        y += 1;
        if (isInValid(x, y)) {
            y = 0;
            x += 1;
            if (isInValid(x, y)) {
                crops[current_row] = old_string;
                minimum_changes = changes;
                return;
            }
        }
        for (int i = 0; i < 26; i++) {
            if ((!isInValid(x - 1, y) && alphabets.charAt(i) == crops[x - 1].charAt(y))
                    || (!isInValid(x, y - 1) && alphabets.charAt(i) == crops[x].charAt(y - 1))) {
                continue;
            }
            helper(crops, x, y, alphabets.charAt(i), changes);
        }
        crops[current_row] = old_string;
    }

    public static int replant(String[] crops) {
        // Write your code here
        // Return the number of replanted crops
        for (int i = 0; i < 26; i++) {
            helper(crops, 0, 0, alphabets.charAt(i), 0);
        }

        return minimum_changes;

    }
}